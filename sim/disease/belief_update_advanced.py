...
class AdvancedBeliefConfig:
    ...
    # Cross-claim inhibition: fraction of other-claims' belief mass to subtract
    # from each belief (0 = no inhibition, 1 = strong mutual exclusion).
    cross_claim_inhibition: float = 0.2
    # If an agent's belief in a truth claim exceeds this threshold, protect them
    # from being converted to other claims (lock-in truth).
    truth_protection_threshold: float = 0.8
...
def advanced_belief_update(
    ...
    # =========== STEP 11: CROSS-CLAIM INHIBITION ==========
    # If enabled, increase mutual exclusivity between claims: when an agent
    # holds belief in some claims, reduce belief in the others proportionally.
    inh = getattr(cfg, "cross_claim_inhibition", 0.0)
    if inh and new_beliefs.shape[1] > 1:
        # Sum of other-claim beliefs for each agent and claim
        total_per_agent = new_beliefs.sum(dim=1, keepdim=True)
        others_sum = total_per_agent - new_beliefs
        # Distribute inhibition equally across claims (normalized by number of other claims)
        denom = max(1, new_beliefs.shape[1] - 1)
        inhibition = inh * (others_sum / float(denom))
        new_beliefs = new_beliefs - inhibition
        new_beliefs = torch.clamp(new_beliefs, 0.0, 1.0)

    # Optional hard mutual exclusion: enforce that each agent can only hold belief
    # in a single claim (the claim with the highest belief). This makes claims
    # strictly incompatible.
    if getattr(cfg, "mutual_exclusion_hard", False) and new_beliefs.shape[1] > 1:
        n_agents = new_beliefs.shape[0]
        max_idx = torch.argmax(new_beliefs, dim=1)
        mask = torch.zeros_like(new_beliefs)
        mask[torch.arange(n_agents), max_idx] = 1.0
        new_beliefs = new_beliefs * mask
        new_beliefs = torch.clamp(new_beliefs, 0.0, 1.0)

    # =========== TRUTH PROTECTION: LOCK-IN ==========
    # Agents who have sufficiently high belief in any true claim are protected
    # from being converted to other claims. We use the narrative state's
    # `ground_truth` flags to identify truth claims.
    try:
        thresh = getattr(cfg, "truth_protection_threshold", None)
        ground_truth_tensor = arch.narrative_state.ground_truth
        if thresh is not None and ground_truth_tensor is not None and new_beliefs.shape[1] > 1:
            truth_mask = ground_truth_tensor > 0.5
            if truth_mask.any():
                truth_beliefs = new_beliefs[:, truth_mask]
                max_true, arg = torch.max(truth_beliefs, dim=1)
                protected = max_true >= float(thresh)
                if protected.any():
                    # Zero non-truth beliefs for protected agents
                    non_truth_mask = ~truth_mask
                    indices = torch.nonzero(protected).squeeze(1)
                    if non_truth_mask.any():
                        new_beliefs[indices[:, None], non_truth_mask.unsqueeze(0).expand(len(indices), -1)] = 0.0
                    # Preserve the chosen truth belief value
                    truth_idx = torch.nonzero(truth_mask).squeeze(1)
                    chosen = truth_idx[arg]
                    new_beliefs[indices, chosen] = max_true[protected]
    except Exception:
        pass
