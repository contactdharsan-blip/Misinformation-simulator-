...
class TownConfig(BaseModel):
    ...
    # Optional fields for explicit town definitions (city-specific inputs)
    city: str | None = None
    data_source: str | None = None
    # If provided, a list of neighborhood specification dicts (name, population, demographics...)
    neighborhood_specs: List[Dict[str, Any]] = Field(default_factory=list)
    # Auto-scale neighborhoods based on population when True
    auto_scale_neighborhoods: bool = True
    # Target approximate population per neighborhood when auto-scaling
    target_neighborhood_size: int = 400
...
class WorldConfig(BaseModel):
    ...
    # Intensity of organized truth/fact campaigns deployed via institutions
    truth_campaign_intensity: float = 0.2
...
class BeliefUpdateConfig(BaseModel):
    ...
    # If true, enforce hard mutual exclusion: each agent may only believe one claim (winner-take-all)
    mutual_exclusion_hard: bool = False
    # If an agent's belief in a truth claim exceeds this threshold, protect them
    # from being converted to other claims (i.e., lock-in truth).
...
class StrainConfig(BaseModel):
    ...
    # Whether the claim is true (factual) or misinformation
    is_true: bool = False
    # How contagious / likely people are to share this claim (multiplicative)
    virality: float = 1.0
    # Long-term persistence of belief in this claim (0.0=no effect, 0.5=halves decay)
